...
export default function Recommendations() {
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      setForm(EMPTY_FORM)
      setShowForm(false)
      setMsg('âœ… Recommendation added!')
      load()
    } catch (err) {
      ...
      <main className="main-content">
        <div className="page-header">
          <div className="page-header-left">
            <h1>ðŸ’¡ <span>Recommendations</span></h1>
            <p>Energy-saving tips personalised for your home.</p>
          </div>
          {role === 'homeowner' && (
            <button className="btn btn-primary" onClick={() => setShowForm(v => !v)}>
              {showForm ? 'âœ• Cancel' : '+ Add Tip'}
            </button>
          )}
        </div>
        ...
        <div className="mini-stats" style={{ marginBottom: 20 }}>
          ...
          <div className="mini-stat">
            <span className="mini-stat-val text-green">Â£{totalSaving}</span>
        ...
        {role === 'homeowner' && showForm && (
          <div className="card" style={{ marginBottom: 20 }}>
            <div className="card-header"><span className="card-title">âž• Add Recommendation</span></div>
            <form onSubmit={handleCreate}>
              ...
              <div className="form-group">
                ...
                <textarea className="form-control" rows={3} required value={form.description} onChange={e => setForm(f=>({...f,description:e.target.value}))} placeholder="Describe the energy-saving tipâ€¦" style={{ resize:'vertical' }} />
              </div>
              <div className="form-group">
                <label className="form-label">Estimated Saving (Â£/month)</label>
              ...
              <button className="btn btn-primary" type="submit" disabled={saving}>{saving ? 'Savingâ€¦' : 'âœ… Save'}</button>
            </form>
          </div>
        )}
        ...
        <div className="card">
          <div className="card-header"><span className="card-title">ðŸ’¡ Your Recommendations</span></div>
          {loading ? (
